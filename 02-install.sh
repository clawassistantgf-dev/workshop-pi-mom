...
echo "└───────────────────────────────────────────────────┘"
echo
echo "  Durée estimée : ~8-16 minutes (selon CPU / réseau — npm workspaces puis build)"
echo "─────────────────────────────────────────────────────"
echo
...
echo
echo "─────────────────────────────────────────────────────"
echo "  🦞 Build pi-mom Les Fadas (ciblé : ai → agent → tui → coding-agent → mom)"
echo "─────────────────────────────────────────────────────"
echo
echo "  ℹ️  pi-tui doit être compilé avant pi-coding-agent (imports TypeScript)."
echo
...
build_targeted() {
  ...
  for rel in packages/ai packages/agent packages/tui packages/coding-agent; do
    if [ ! -f "$PI_MONO_DIR/$rel/package.json" ]; then
      echo "⚠  Chemin monorepo inattendu : $PI_MONO_DIR/$rel (build ciblé impossible)."
      return 1
    fi
  done
  for rel in packages/ai packages/agent packages/tui packages/coding-agent; do
...
if ! build_targeted; then
  echo "⏳ Repli : npm run build à la racine du monorepo (complet, ordre géré par le repo)."
  cd "$PI_MONO_DIR"
  npm run build
fi
