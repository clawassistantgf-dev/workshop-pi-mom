...
| **`docs/`** | **Page d’aide** : [`docs/index.html`](docs/index.html) (GitHub Pages depuis `/docs`) — débogage **script 2 · Windows**, copie du prompt + boutons ChatGPT / Claude / Gemini. **Message post-atelier** (texte brut, copier-coller messagerie) : [`docs/MESSAGE-POST-ATELIER.txt`](docs/MESSAGE-POST-ATELIER.txt). **PDF slides** : [`docs/workshop-pi-mom-slides.pdf`](docs/workshop-pi-mom-slides.pdf). Guides : [UPDATE](docs/UPDATE.md), [MISSIONS](docs/MISSIONS.md), [STRUCTURE_GUIDE](docs/STRUCTURE_GUIDE.md). Fichier [`prompt-deblocage-script2-windows-FR.txt`](docs/prompt-deblocage-script2-windows-FR.txt). Ancienne URL `docs/prompt-script2-windows/` → redirection vers l’accueil. |
